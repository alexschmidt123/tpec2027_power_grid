...
\maketitle
\begin{abstract}
Stable power-system operation benefits from reduced uncertainty in inertia
and frequency-response parameters. Active probing helps identify these
parameters, but its information yield depends on the inputs. We apply
sequential Bayesian optimal experimental design to adapt probe durations
to observed responses and obtain more information with the same number
of probes. On IEEE9 and IEEE14 reduced models, we compare
offline-trained adaptive policies (DAD and RL-sBOED), online policy refinement
(Step-DAD), one-step Bayesian design (Myopic), an optimized fixed sequence,
and random probing. With three to five probes, the learned
methods achieve higher mean EIG than the three
baselines. DAD and RL-sBOED also require substantially less online computation
than Myopic and Step-DAD. Online refinement gives modest additional gains
on IEEE9 but no consistent improvement on IEEE14, where estimator saturation
limits method discrimination. The results support offline-trained sequential
design for informative probing with low online computation.
\end{abstract}
\begin{IEEEkeywords}
