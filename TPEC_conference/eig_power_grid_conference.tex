...
Power-grid stability is vital to industry and daily life. Stability
assessment and control require understanding disturbance responses,
which depend on uncertain inertia $M$ and effective frequency response
$K$. Identifying these parameters supports informed operating
decisions~\cite{peng2024probing}.

Active probing obtains parameter information by injecting small,
controlled signals and measuring frequency responses~\cite{peng2024probing}.
The signal design determines what can be learned. We seek more
information from a limited probing budget by selecting pulse durations
while retaining physical state between experiments.

Predetermined sequences cannot adapt to incoming measurements. Myopic
Bayesian optimization uses feedback but greedily maximizes only next-step
information gain, ignoring subsequent learning
opportunities~\cite{huan2016sequential}, and requires repeated online
search. The gap is efficient adaptation that considers the complete
probing sequence.

We apply adaptive sequential Bayesian optimal experimental design.
Policies use observed responses to guide later probes, while full-sequence
optimization accounts for how early measurements inform subsequent choices.
DAD~\cite{foster2021dad} and RL-sBOED~\cite{blau2022rlsboed} learn policies
offline, replacing online optimization with policy evaluation.
Step-DAD~\cite{hedman2025stepdad} adds refinement using the observed history.

Our contribution is the grid-probing formulation and matched evaluation
of these existing methods on reduced IEEE9 and IEEE14 models.
At equal probe budgets, all three learned methods achieve higher mean
estimated information gain than Myopic, an optimized fixed sequence,
and random probing at every tested horizon. DAD and RL-sBOED also
substantially reduce online decision time relative to Myopic.
These results demonstrate improved information acquisition for the
broader purpose of informed stability assessment and control.
